...
public class FibPair {
    ...
    private static void nextText( Integer bigger
        ...
        Pair<Integer> tempPair = new Pair<>(bigger, smaller);
        Pair<Integer> newPair = new Pair<>(tempPair.getFirst() + tempPair.getSecond(), tempPair.getFirst());
    ...
    private static Pair<Integer> nextPairAfter(Pair<Integer> pair) {
        return new Pair<Integer>(pair.getFirst() + pair.getSecond(), pair.getFirst());
    }
    ...
    private static Integer fib( int n) {
      return fib_helper(n, new Pair<Integer>(0, 1));
    }

    private static Integer fib_helper(int n, Pair<Integer> currentPair) {
      if (n == 0) return currentPair.getFirst();
      return fib_helper(n-1, nextPairAfter(currentPair));
    }
}
